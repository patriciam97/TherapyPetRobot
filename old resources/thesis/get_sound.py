...
def create_sound(sound, status):
    title = '/Users/patriciamilou/Documents/Github/TherapyPetRobot/old resources/sound_' + \
        str(status)+'.wav'
    ...
    for x, y in sound:
        y = int(x/level) if (int(x/level) < 32767 and int(x/level)
                             > -32767) else (-32767 if x < 0 else 32767)
    ...
    print("sound_"+str(status)+".wav updated.")
    noise_output.close()
